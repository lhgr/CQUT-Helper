...
| **今日一言**    | 在今日页展示可在线维护的短句，支持每日轮换、节日优先与离线缓存          |
...
- 今日页底部的一言支持每日轮换和手动切换，节日等限时内容可优先展示。
...
> \[!CAUTION]
> v1.0.0 起启用新的长期正式签名，无法从 v0.3.0 及更早版本直接覆盖安装。
> 请先确认账号密码可用，再卸载旧版本并重新安装；卸载会清除旧版本保存在本机的登录状态、课表缓存和设置。
